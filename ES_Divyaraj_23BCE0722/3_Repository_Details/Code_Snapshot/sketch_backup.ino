...
float gyroX = 0, gyroY = 0, gyroZ = 0;  // Add gyro data
...
int tempStatus = 0;
int vibClass = 3;
int lastVibClass = 3;  // Track previous classification
float vibConf = 0;
int stableCount = 0;  // Stability counter
...
int impactCount = 0;
int roughCount = 0;
int loopCount = 0;  // Track every loop
...
void extractVibrationFeatures(float& magnitude, float& jerk, float& variance,
  ...
  // Noise threshold for Wokwi - treat tiny values as zero
  if (rawMag < 0.05) {
    magnitude = 1.0;
    rawMag = 0.0;  // Force to zero for stability
  } else {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (m < 0.05)  // Same noise threshold
      m = 1.0;
    else
      m = 1.0 + m;
    sum += m;
  ...
  // If variance is tiny, force to zero (no movement)
...
void setup() {
  ...
  Serial.println(
      "Loop | Pot1 Pot2 | Temp  | AccelRaw(X,Y,Z) | GyroRaw(X,Y,Z) | AccelFilt "
      "| Mag  Jrk Var | Class");
  Serial.println(
      "------------------------------------------------------------------------"
      "--------------------------");
...
void loop() {
  ...
  // HARD OVERRIDE: If nearly zero movement, force STATIC
  float rawMag = sqrt(accelX * accelX + accelY * accelY + accelZ * accelZ);
  if (rawMag < 0.08 && jerk < 0.5 && vibVar < 0.005) {
    vibClass = 3;  // Force STATIC
    vibConf = 0.95;
    stableCount = 0;
  } else {
    int newVibClass =
    ...
    // Stability filter: only change class if consistent for 3 readings
  ...
  // Print debug info EVERY loop iteration
  loopCount++;

  // Calculate filtered accel magnitude (after noise threshold)
  float rawMag = sqrt(accelX * accelX + accelY * accelY + accelZ * accelZ);
  float filtMag = (rawMag < 0.05) ? 0.0 : rawMag;

  Serial.print(loopCount);
  Serial.print(" | ");

  // Potentiometer raw values
  Serial.print(potInt);
  Serial.print(" ");
  Serial.print(potAmb);
  Serial.print(" | ");
